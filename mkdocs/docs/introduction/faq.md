...
The vocabulary is presented as Linked Open Data using the Resource Description Framework (RDF) family of languages, and a Class and Properties (loosely UML) diagram. We want to support RDF based and non-RDF based uses of the vocabulary, basically any way that people want to use software and data on the internet to help create economic networks. Partially complete options are a Json-schema reference, and a GraphQL reference.  See [the Specification overview page](../specification/spec-overview.md) for up-to-date information.
