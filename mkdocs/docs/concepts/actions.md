...
*In the above chart, the `notApplicable` values are not included, for easier overall viewing.  For the complete list of behavior values by action as defined in the rdf vocabulary, see the [turtle file starting at the Actions section](hhttps://codeberg.org/valueflows/pages/src/branch/main/assets/all_vf.TTL#L1289).
